...
/**
 * 类JsonRpcClient.java的实现描述：
 *
 * @author xuhao create on 2020/12/8 17:12
 */
...
public class JsonRpcClient {
    ...
    public synchronized <T> T invokeAndReadResponse(String methodName, Object argument, Class<T> clazz, OutputStream output, InputStream input) throws Throwable {
    ...
    protected void handleErrorResponse(ObjectNode jsonObject) throws Throwable {
        if (hasError(jsonObject)) {
            ...
            if (exceptionResolver == null) {
                ...
                Throwable throwable = exceptionResolver.resolveException(jsonObject);
                if(throwable!=null){
                    throw exceptionResolver.resolveException(jsonObject);
                }else {
                    ObjectNode errorObject = ObjectNode.class.cast(jsonObject.get(JsonRpcBasicServer.ERROR));
                    throw  new Throwable(errorObject.toString());
                }
            }
        }
    }
    ...
    private boolean hasNonNullData(final ObjectNode node, final String key) {

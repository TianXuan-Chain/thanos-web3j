...
public class ThanosEthTransaction extends BaseTransaction {
    ...
    //for test
    public ThanosEthTransaction(Set<ByteArrayWrapper> executeStates) {
        super(null);
        this.executeStates = executeStates;
        this.executeStatesBytes = RLP.encodeSet(this.executeStates);
    }

    public ThanosEthTransaction(byte[] rawData) {
        super(rawData);
    }

    public static ThanosEthTransaction convertFrom(RawTransaction rawTransaction) {
        if (rawTransaction == null) {
            return null;
        }
        Set<ByteArrayWrapper> executeStates = new HashSet<>();
        if (CollectionUtils.isNotEmpty(rawTransaction.getExecuteStates())) {
            for (String es : rawTransaction.getExecuteStates()) {
                executeStates.add(new ByteArrayWrapper(es.getBytes()));
            }
        }
        byte[] receiveAddress = null;
        if (StringUtils.isNotBlank(rawTransaction.getTo())) {
            receiveAddress = Numeric.hexStringToByteArray(rawTransaction.getTo());
        }
        byte[] data = Hex.decode(rawTransaction.getData());
        return new ThanosEthTransaction(rawTransaction.getPublicKey(), rawTransaction.getNonce().toByteArray(), rawTransaction.getFutureEventNumber(),
                rawTransaction.getGasPrice().toByteArray(), rawTransaction.getGasLimit().toByteArray(), receiveAddress, rawTransaction.getValue().toByteArray(), data, executeStates, null);

    }

    public ThanosEthTransaction(byte[] publicKey, byte[] nonce, long futureEventNumber, byte[] gasPrice, byte[] gasLimit, byte[] receiveAddress, byte[] value, byte[] data, Set<ByteArrayWrapper> executeStates, byte[] signature) {
    ...
    public ThanosEthTransaction(byte[] publicKey, byte[] nonce, long futureEventNumber, byte[] gasPrice, byte[] gasLimit, byte[] receiveAddress, byte[] value, byte[] data, Set<ByteArrayWrapper> executeStates, byte[] signature, byte[] hash, byte[] rlpEncode) {
        super(publicKey, futureEventNumber, nonce, signature);
        this.gasPrice = gasPrice;
        this.gasLimit = gasLimit;
        this.receiveAddress = receiveAddress;

...
- [4. NETEASE CHAIN系统合约部署](#4-netease-chain系统合约部署)
...
web3sdk是用来访问netease-chain节点的java API。<br />
主要由两部分组成：<br />
(1) AMOP（链上链下）系统旨在为联盟链提供一个安全高效的消息信道。<br />
(2) web3j(Web3 Java Ethereum Ðapp API),这个部分来源于[web3j](https://github.com/web3j/web3j)，并针对netease的做了相应改动。主要改动点为：

① 交易结构中增加了randomid和blocklimit，这个改动对于sdk的使用者是透明的。<br /> ②为web3增加了AMOP消息信道。<br />

本文档主要说明如何使用AMOP消息信道调用web3j的API。AMOP（链上链下）的使用可以参考AMOP使用指南。web3j的使用可以参考[web3j](https://github.com/web3j/web3j)和[存证sample](https://github.com/netease-chain/evidenceSample)。
...
- **[提供系统合约部署和使用工具](##4-netease-chain系统合约部署)**
- **[使用国密算法发交易](doc/guomi_support_manual.md)**：[国密版NETEASE CHAIN](https://github.com/netease-chain/netease-chain/blob/master/doc/国密操作文档.md) 使用国密算法验证交易，客户端相应地需使用国密算法对交易进行签名，web3sdk客户端为国密版NETEASE CHAIN提供了支持，具体使用方法可参考文档[《web3sdk对国密版NETEASE CHAIN的支持》](doc/guomi_support_manual.md)
...
1、需要首先搭建NETEASE CHAIN区块链环境（快速搭建可参考[NETEASE CHAIN一键快速安装部署](https://github.com/netease-chain/netease-chain/tree/master/sample)）。<br />
...
web3sdk客户端证书ca.crt, client.keystore生成方法请参考[netease-chain区块链操作手册的生成sdk证书](https://github.com/netease-chain/netease-chain/tree/master/doc/manual#24-生成sdk证书)一节。<br>
具体步骤可以参考[sdk.sh](https://github.com/netease-chain/netease-chain/blob/master/cert/sdk.sh),详细解释如下：<br>
...
| 配置文件                   | 详细说明                                     |
| ---------------------- | ---------------------------------------- |
| applicationContext.xml | 主要包括三项配置: <br> - encryptType: 配置国密算法开启/关闭开关，0表示不使用国密算法发交易，1表示开启国密算法发交易，默认为0(即不使用国密算法发交易，web3sdk支持国密算法的具体方法可参考文档[web3sdk对国密版NETEASE CHAIN的支持](https://github.com/netease-chain/web3sdk/blob/master/doc/guomi_support_manual.md));  <br>  - systemProxyAddress: 配置NETEASE CHAIN系统合约地址, 部署系统合约成功后，要将systemProxyAddress对应的值改为部署的系统合约地址;  <br> - privKey: 向NETEASE CHAIN节点发交易或发消息的账户私钥，使用默认配置即可 <br> - ChannelConnections：配置NETEASE CHAIN节点信息和证书，证书相关配置如下:  <br>  (1) caCertPath: CA证书路径，默认为dist/conf/ca.crt; <br> (2) clientKeystorePath: 客户端证书路径，默认为dist/conf/client.keystore; <br> (3)keystorePassWord: 客户端证书文件访问口令, 默认为123456; <br> (4) clientCertPassWord: 客户端证书验证口令, 默认为123456 |
| ca.crt                 | 用来验证节点或者前置的CA证书，必须与链上NETEASE CHAIN节点CA证书保持一致           |
...
# 4. NETEASE CHAIN系统合约部署

部署完适配于[NETEASE CHAIN](https://github.com/netease-chain/netease-chain/tree/master/doc/manual)的web3sdk后，可调用InitSystemContract部署系统合约，系统合约部署工具InitSystemContract由`src/main/java/org/netease/contract/tools/InitSystemContract.java`调用合约生成的java代码实现，使用如下命令部署系统合约：
...
执行完上述两步操作后，按照[NETEASE CHAIN区块链操作手册](https://github.com/netease-chain/netease-chain/tree/master/doc/manual#第七章-多记账节点组网) **多记账节点组网**一节进行加入新节点等操作。
...
在SDK工具包/mydata/web3sdk/dist/bin（设web3sdk存放于/mydata目录下）目录下，compile.sh为合约编译脚本，web3sdk为SDK的的执行脚本。web3sdk脚本中将系统合约接口进行暴露。系统合约介绍文档参考[NETEASE CHAIN系统合约介绍](https://github.com/netease-chain/Wiki/tree/master/netease-chain%E7%B3%BB%E7%BB%9F%E5%90%88%E7%BA%A6%E4%BB%8B%E7%BB%8D)
...
```
**功能**：将${node_json_path}指定的节点从NETEASE CHAIN区块链网络中退出(注: ${node_json_path}是节点配置文件相对于/mydata/web3sdk/dist/conf的路径，节点配置文件说明同上)
...
web3sdk提供了一些测试工具，方便确定web3sdk与[NETEASE CHAIN](https://github.com/netease-chain/netease-chain)通信是否正常，本节简要介绍这些测试工具使用方法：
...
#调用测试合约TestOk
$ java -cp 'conf/:apps/*:lib/*' com.thanos.web3j.channel.test.TestOk
===================================================================
=====INIT ECDSA KEYPAIR From private key===
...
$ cd /mydata/web3sdk/dist
#调用测试合约Ethereum
$ java -cp 'conf/:apps/*:lib/*' com.thanos.web3j.channel.test.Ethereum
start...
===================================================================
=====INIT ECDSA KEYPAIR From private key===
...
com.thanos.web3j.console.Web3RpcApi
...
在SDK工具包/dist/bin目录下，compile.sh为合约编译脚本，web3sdk为SDK的的执行脚本。web3sdk脚本中将权限相关的接口进行暴露，使用相关命令执行即可。权限控制介绍文档请参看[NETEASE CHAIN权限模型（ARPI）介绍] 、[联盟链的权限体系](https://github.com/netease-chain/Wiki/tree/master/%E5%8C%BA%E5%9D%97%E9%93%BE%E7%9A%84%E6%9D%83%E9%99%90%E4%BD%93%E7%B3%BB) 。

```java
com.netease.contract.tools.ARPI_Model    #一键执行类
com.netease.contract.tools.AuthorityManagerTools
```
...
(2) 安装netease-solc,netease-solc为solidity编译器.

  将下载下来的netease-solc，将netease-solc拷贝到/usr/bin目录下，执行命令chmod +x netease-solc。如此netease-solc即安装完成。
...
- 生成兼容国密版netease-chain的java代码具体方法可参考[web3sdk对国密版NETEASE CHAIN的支持的生成支持国密算法的java代码](https://github.com/netease-chain/web3sdk/blob/master/doc/guomi_support_manual.md#6-%E7%94%9F%E6%88%90%E6%94%AF%E6%8C%81%E5%9B%BD%E5%AF%86%E7%AE%97%E6%B3%95%E7%9A%84java%E4%BB%A3%E7%A0%81)一节
...
下面代码演示了通过web3sdk调用合约向netease-chain发交易的主要流程：

```java
	package com.thanos.web3j.channel.test;
...
**(1) 初始化web3j对象，连接到netease-chain节点**

web3sdk使用AMOP（链上链下）连接netease-chain节点：

```java
	//初始化AMOP的service，初始化函数详见配置文件，sleep(3000)是确保AMOP网络连接初始化完成。注意：com.thanos.web3j.channel.client.Service在Java Client端须为单实例，否则与链上节点连接会有问题
    Service service = context.getBean(Service.class);
    service.run();
    Thread.sleep(3000);
```
...
> web3sdk也支持通过http和ipc来初始化web3j对象，但在fisc-netease中推荐使用AMOP
...
> 样例给出的是新构建一个私钥文件。web3sdk也可以用证书来初始化交易签名私钥，对交易进行签可参考[存证sample](https://github.com/netease-chain/evidenceSample)。
...
从 https://github.com/netease-chain/web3sdk 下载代码（Download ZIP），解压之后，目录名称将为：web3sdk-master，运行gradle命令后生成的工具包中，/dist/apps目录下生成的jar包名称为web3sdk-master.jar，导致和/dist/bin/web3sdk中配置的CLASSPATH中的配置项$APP_HOME/apps/web3sdk.jar名称不一致，从而导致使用工具包生成合约Java Wrap代码时会报错。
...
(5) [netease-chain中client.keystore 的生成方法](https://github.com/netease-chain/web3sdk/issues/20)
